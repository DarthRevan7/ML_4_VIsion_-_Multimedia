...
from main_start_au import margin, lr_clean, n_epochs
...
# Parametri correnti assegnati a costanti locali (formattati come nel train)
MARGIN_VAL_ORIGINAL = margin
TARGET_MARGIN = f"0{int(margin*10)}"
TARGET_LR = lr_clean

# DB Paths e File di Output Master dedicato all'augmentation
logodet_path = "databases\\LogoDet-3K"
flicker_path = "databases\\FlickrLogos32"
output_csv = "results\\checkpoints_master_summary_aug.csv"
...
def run_evaluation():
    ...
    print("\n" + "="*60)
    print("🎯 PARAMETRI ACCETTATI DA MAIN_START_AU:")
    ...
    # Regex adattata per intercettare i file del tipo: checkpoint_epoch_X_of_10_aug.pth
    pattern = re.compile(rf"^checkpoint_epoch_(\d+)_of_{n_epochs}_aug\.pth$")
    ...
    if os.path.exists(checkpoint_dir):
        for filename in os.listdir(checkpoint_dir):
            ...
            if match:
                ...
                # Passiamo TARGET_LR e TARGET_MARGIN per salvare correttamente i metadati nel CSV storico
                checkpoint_list.append((epoch_num, TARGET_LR, TARGET_MARGIN, full_path))

    if not checkpoint_list:
        print(f"❌ Nessun checkpoint trovato per il formato atteso (^checkpoint_epoch_X_of_{n_epochs}_aug.pth)")
        return
    ...
    # ----------------------------------------------------------------------
    # PHASE 5: RECAP COMPATTO AD ODNI FINE COMPLETAMENTO RUN
    # ----------------------------------------------------------------------
    if os.path.exists(output_csv):

...
result_file_path = "results\\final_eval_logonet_resnet50_margin05_E10_LR2.5e-05.csv"
...
def calculate_metrics_and_plots(q_embs, q_labels, g_embs, g_labels, dataset_name, ks=[1, 5, 10]):
    ...
    plt.title(f"CMC Curve - {dataset_name} - margin05_E10_LR2.5e-05")
    plt.xlabel("Rank")
    plt.ylabel("Identification Probability")
    plt.grid(True)
    plt.savefig(f"results\\cmc_{dataset_name}_margin05_E10_LR2.5e-05.png")
    plt.close()
